...
class EsiResolver:
    ...
    def _resolve_batch(self, batch: List[int]) -> Tuple[Dict[int, str], List[int]]:
        ...
        self.logger.warning("esi non-fatal error status=%s batch_size=%s", status_code, len(batch))
        return {}, batch
    ...
    def _post_with_retries(self, batch: List[int]) -> requests.Response | None:
        for attempt in range(self.max_transient_retries + 1):
            ...
            if response.status_code in {408, 409, 420, 429, 500, 502, 503, 504}:
                if attempt >= self.max_transient_retries:
                    return response
                self.stats["transient_retries"] += 1
    ...
    def _parse_success(self, batch: List[int], response: requests.Response) -> Tuple[Dict[int, str], List[int]]:
        resolved: Dict[int, str] = {}
    ...
    def _inspect_error_limit(self, response: requests.Response) -> None:
        ...
        try:
            ...
            remain_int = None
        if remain_int is not None:
            current = self.stats["max_error_limit_seen"]
            if current is None or remain_int < current:
                self.stats["max_error_limit_seen"] = remain_int
            if remain_int <= 5:
                self.logger.warning(
